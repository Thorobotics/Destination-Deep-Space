...
# Credit to Screaming Chickens 3997 for base calculation code
# Credit to Dakota for getting it to work
...
from cscore import CameraServer, VideoSource, VideoCamera
...
def threshold_video(frame):
    ...
    # define range of turquoise in HSV
    lower_color = np.array([60,108,46])
...
def findTargets(contours, image, centerX, centerY):
    ...
    targets = []
    yawToTarget = 0
    vt = NetworkTables.getTable("VisionTable")
    if len(contours) >= 2:
        ...
        for i in range(len(biggestCnts) - 1):
            ...
            # If contour angles are opposite
                ...
                pitchToTarget = calculatePitch(centerOfTargetY, centerY, V_FOCAL_LENGTH)

                #Make sure no duplicates, then append
                buffer = 1
                if [centerOfTarget, yawToTarget] not in targets:
                    targets.append([centerOfTarget, yawToTarget])
                    if biggestCnts[i][1] - biggestCnts[i + 1][1] > buffer:
                        vt.putNumber("Slide", 1.0)
                        continue
                    if biggestCnts[i + 1][1] - biggestCnts[i][1] > buffer:
                        vt.putNumber("Slide", -1.0)
                        continue
                    vt.putNumber("Slide", 0.0)
    ...
    if (len(targets) > 0):
        ...
        #             (255, 255, 255))
        # cv2.line(image, (finalTarget[0], screenHeight), (finalTarget[0], 0), (255, 0, 0), 1)
        # cv2.putText(image, "Pitch: " + str(pitchToTarget), (40, 80), cv2.FONT_HERSHEY_COMPLEX, .6,
        #            (255, 255, 255))
        cv2.line(image,(screenWidth, math.floor(centerOfTargetY)), (0, math.floor(centerOfTargetY)), (0,0,255) , 1)

        #height(frame-1/2)
        #TODO send currentAngleError to robot program
        currentAngleError = finalTarget[1]
        vt.putNumber("Yaw", currentAngleError)
        vt.putNumber("Pitch", pitchToTarget)
    else:
        vt.putNumber("Yaw", 0.0)
        vt.putNumber("Pitch", 0.0)
        vt.putNumber("Slide", 0.0)
    cv2.line(image, (round(centerX), screenHeight), (round(centerX), 0), (255, 255, 255), 2)
    cv2.line(image, (screenWidth, round(centerY)), (0, round(centerY)), (255, 255, 255), 2)
...
def startCamera(config):
    ...
    #VideoCamera.setExposureManual(camera, 0.1)
